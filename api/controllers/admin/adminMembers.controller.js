...
function post(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (await dbModel.adminMembers.countDocuments({ organization: sessionDoc.organization, username: data.username }) > 0)
        return reject(`username already exists`)

      data.organization = sessionDoc.organization
      const newDoc = new dbModel.adminMembers(data)
...
function put(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let doc = await dbModel.adminMembers.findOne({ organization: sessionDoc.organization, _id: req.params.param1 })
      if (!doc) return reject(`member not found`)

      data.organization = sessionDoc.organization
      doc = Object.assign(doc, data)

      if (await dbModel.adminMembers.countDocuments({ organization: sessionDoc.organization, username: doc.username, _id: { $ne: doc._id } }) > 0)
        return reject(`username already exists`)
...
function deleteItem(dbModel, sessionDoc, req) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbModel.adminMembers.removeOne(sessionDoc, { organization: sessionDoc.organization, _id: req.params.param1 })
        .then(resolve)
        .catch(reject)
    } catch (err) {
      reject(err)
